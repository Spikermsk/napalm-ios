...
def test_remote_file_size(scp_fixture):
    ssh_conn, scp_transfer = scp_fixture
    remote_file_size = scp_transfer._remote_file_size()
    assert remote_file_size == 9

def test_md5_methods(scp_fixture):
    ssh_conn, scp_transfer = scp_fixture
    md5_value = 'd8d77109f4a24efc3bd53d7cabb7ee35'

    remote_md5 = scp_transfer._remote_md5()
